...
\begin{itemize}
    \item 删减并整理文档类代码, 从原来的 2400 多行简化为 1750 行. 补充了大量代码注释, 增加文档类的可读性.
    \item 删除了大量非必需的盒子. 如各种 ascolorbox. 这些盒子可能是用来作为主要数学环境盒子的替代或备用品. 
    ...
    \item 重置了字体选项, 现在可以用 font 选项同时更改西文正文和数学字体. stix2 和 termes 选项的字体采用 unicode 编码, 字符全面.
    \item 大幅度调整了颜色主题. 
    \item 规范了定理环境的用法, 恢复了amsthms 默认的标签引用方式, 从而避免了因使用习惯的不同而产生的大量警告. 
    \item 修正了魔改版证明环境无法使用 \texttt{\textbackslash qedhere} 命令修改证毕符号位置的问题.
    \item 重做了目录样式, 使其更接近严肃出版物. chapter 改用汉字计数. subsection 在正文和目录中均不再计数\footnote{可通过 config.tex 中的 \textbackslash setcounter 命令调整}, 规范了无编号章节的使用.
\end{itemize}

    \subsection{编译环境}

本模板经测试可在 Win11 + TexLive 2025 + VSCode LaTeX Workshop 2025 的编译环境下编译 (在线编辑器也可). 中文模式必须用 Xe\LaTeX{}编译, 西文模式用 Xe\LaTeX 与 PDF\LaTeX{}均可. 后者编译速度更快, 但对字体的支持不如 Xe\LaTeX 全面, 不过也基本够用. 

本模板的编译速度相较魔改版的提升可能有限, 作者未进行仔细比较. 如有速度需求, 请尽量少编译图片, 尤其是 part 和 chapterhead.
...
作者认为拥有良好的文件管理意识对于 \LaTeX{} 文章的编写极为重要, 因此本模板的文档管理较为严格, 为章节、图片、宏包等分别单开一级子文件夹. 子文件夹层级不要太复杂, 通常只要一级就够了. 本模板的根目录结构如下: 
...
西文字体的 font 选项提供了 Computer Modern 系列 (cm), STIX Two 系列 (stix2), TeX Gyre Termes 系列 (termes) 和 Garamond 系列 (garamond) 字体. 如果使用者对这一搭配不满意, 可在文档类中查找 \texttt{\textbackslash setmainfont} 命令来修改, 或者使用 nofont 模式, 自己重定义西文字体. 请注意, nofont 中未引入数学字符宏包 (如 amssymb) 等, 这是考虑到使用者未必需要数学字符, 或者希望使用 unicode-math 宏包, 此宏包与 amssymb 等宏包是冲突的. 
...
计数器选项为 thmcnt, 默认值为 section, 统一将各定理环境按节编号. 如需要按章编号, 则需设置 thmcnt=chapter.

本模板支持 hyperref 超链接.

\section{参考文献与索引}

参考文献\index{cankaoewenxian@参考文献}的编译方法有两种: 一是用\texttt{thebibliography} 环境编译, 好处是代码较简单, 不需要使用 biblatex 编译, 缺点是顺序需要手动调整. 二是用 biblatex 编译, 全自动管理但代码较复杂. 由于讲义对参考文献的引用顺序和格式不要求像论文那样严谨, 甚至不一定要有参考文献, 因此本模板使用第一种方法编译. 正确引用可能需要编译两次. 这是一条引用 \cite{xiong}.

索引需要使用 \texttt{imakeidx} 宏包和 \texttt{\textbackslash makeindex} 命令. 调整索引样式的一个方法是建立一个 indexstyle.ist 文件, 并用 
\begin{center}
\texttt{\textbackslash makeindex[options=-s indexstyle.ist]}
\end{center}
来调用. 在作者的 VSCode\index{VScode} 测试环境中, 如果用 latexmk 编译, 可能无法正确处理索引样式, 这时需要加入 makeindex 配方, 按 latexmk \( \to \) makeindex \( \to \) latexmk 分三次编译. 由于作者不太懂编译器原理, 所以就不献丑了. 

这是一条索引\index{suoyin@索引}. 这是一条分级索引 \index{fenjisuoyin@分级索引}. 分级\index{fenjisuoyin@分级索引!fenji@分级}.
