...
设 $ f(x)=\sum_{i=1}^{k} a_i \chi_{A_i}(x)$ 是 $E$ 上的\emph{非负简单函数}，中文其中
...
\begin{lstlisting}
\begin{definition}[可积性] \label{def:int}
设 $ f(x)=\sum\limits_{i=1}^{k} a_i \chi_{A_i}(x)$ 是 $E$ 上的\textbf{非负简单函数}，其中
$\{A_1,A_2,\ldots,A_k\}$ 是 $E$ 上的一个可测分割，$a_1,a_2,\ldots,a_k$ 是非负实数。定义 $f$
...
本模板默认的公式行距 (displayskip) 是 3pt. 但在少数场景下, 这个行距可能会变得不合适. 作者未能找到全局调整的方法, 因此作者补充定义了两个手动调整公式行距的命令 \texttt{\textbackslash displaynarrow} 和 \texttt{\textbackslash displaywide}. 虽然手段不够优雅, 但效果足够了. 其用法与实例如下:

这是 3pt 间距行间公式. 
\[
(a+b\mathrm{i})^{-1}=\frac{1}{a+b\mathrm{i}}=\frac{a-b\mathrm{i}}{(a+b\mathrm{i})(a-b\mathrm{i})}=\frac{a-b\mathrm{i}}{a^2+b^2}.
\]
这是 6pt 间距行间公式. 
{\displaywide
\[
...
这是 3pt 间距行间公式. 
\[
(a+b\mathrm{i})^{-1}=\frac{1}{a+b\mathrm{i}}=
\frac{a-b\mathrm{i}}{(a+b\mathrm{i})(a-b\mathrm{i})}
...
问题环境 (problem) 创建一个按章标号的题目. 如果希望按节编号, 请找到文档类中这段代码:
\begin{lstlisting}
\newcounter{prob}[chapter]
\end{lstlisting}
并将 chapter 改为 section. 

\begin{problem}
    证明函数 \( f(x) = \sin x \) 在 \( [0, \pi/2] \) 上严格递增.   
\end{problem}

问题环境有 3pt 的上下行距. 适合编写习题集用.

    \subsection{习题环境}

本模板提供了每节习题和每章章末总习题两种习题模式. 每节习题的环境是重做的, 显得更大气. 适合讲义中布置习题.
